...
Each SPN has a dedicated Zenodo record with three zip archives. Files are
downloaded once and cached locally — repeat calls skip the download.
...
Once `get_ground_truth()` has been called, these functions let you access
specific files without navigating the directory structure manually.

### `get_mutations()`

Returns the path to the mutations RDS file for a given sample type, coverage
and purity.
...
Once `get_sarek_results()` has been called, these functions return named lists
of file paths for a given sample, coverage, purity and caller.

### `get_sarek_vcf()`

Returns VCF and index file paths. Supported callers: `"mutect2"`, `"strelka"`,
...
Once `get_tumourevo_results()` has been called, these functions return named
lists of file paths. All require `spn`, `coverage`, `purity`, `vcf_caller`
(`"mutect2"` or `"strelka"`), and `cna_caller` (`"ascat"` or `"sequenza"`).
...
Subclonal deconvolution results. Supported tools: `"mobster"`, `"pyclonevi"`,
`"ctree"`, `"viber"`.
...
`"sparsesignatures"`, `"BASCULE"`. `sigprofiler` also requires a `context`
argument (e.g. `"SBS96"`, `"ID83"`).
